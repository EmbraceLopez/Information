 ...
 * description: 圆形线段进度条
*/
public class LineRotateLoadingView extends View {
    ...
    public LineRotateLoadingView(Context context) {
        this(context, null);
    }

    public LineRotateLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LineRotateLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.LineRotateLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.LineRotateLoadingView_loading_view_size,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,32,getResources().getDisplayMetrics()));
        mPaintColor = array.getInt(R.styleable.LineRotateLoadingView_android_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public LineRotateLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
